...
# Upload prescription image and extract reminder
    ...
    for line in lines:
        for med in medication_info:
            if med.lower() in line.lower():
                reminder_entry.delete(0, tk.END)
                reminder_entry.insert(0, f"Take {med}")
                med_dropdown.set(med)
                risk_dropdown.set(medication_info[med]["risk"])
                benefit_dropdown.set(medication_info[med]["benefit"])
                match = re.search(r'\b(\d{1,2}:\d{2})\b', line)
                if match:
                    time_entry.delete(0, tk.END)
                    time_entry.insert(0, match.group(1))
                return
    messagebox.showinfo("Prescription", "No known medication found in image.")

upload_btn = ttk.Button(add_frame, text="Upload Prescription", command=upload_prescription)
