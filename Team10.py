...
# Medication risk dictionary
medication_risks = {
    "Paracetamol": "Overuse may cause liver damage.",
    "Ibuprofen": "May cause stomach ulcers or kidney issues if taken long-term.",
    "Losartan": "Can cause dizziness or increased potassium levels.",
    "Metformin": "May lead to lactic acidosis in rare cases.",
    "Levothyroxine": "Overdose may cause heart palpitations or anxiety.",
    "Aspirin": "Risk of bleeding, especially with other blood thinners.",
    "Atorvastatin": "May cause muscle pain or liver enzyme elevation.",
    "Vitamin D": "Excess intake may lead to calcium buildup in the blood.",
    "Calcium": "Too much can cause kidney stones or constipation."
}
...
def load_reminders():
    if os.path.exists(REMINDER_FILE):
        with open(REMINDER_FILE, "r") as f:
            ...
            for date in data:
                data[date] = [
                    {"time": "09:00", "text": r, "medication": ""} if isinstance(r, str) else r
                    for r in data[date]
                ]
            return data
    else:
        ...
        sample = {
            today: [
                {"time": "16:45", "text": "Take Paracetamol", "medication": "Paracetamol"},
                {"time": "16:46", "text": "Take Vitamin D", "medication": "Vitamin D"}
            ]
        }
...
def refresh_reminders():
    ...
    if selected_date in reminders:
        for idx, reminder in enumerate(reminders[selected_date]):
            ...
            reminder_text = ttk.Label(frame, text=f"{idx + 1}. {reminder['time']} - {reminder['text']}{med}", font=("Segoe UI", 11))
...
def edit_reminder(index):
    ...
    new_med = simpledialog.askstring("Edit Medication", "Enter medication name (optional):")
    if new_text and new_time:
        reminders[selected_date][index] = {"time": new_time, "text": new_text, "medication": new_med or ""}
...
med_entry = ttk.Entry(add_frame, width=20)
med_entry.pack(side=tk.LEFT, padx=5)
med_entry.insert(0, "Medication")
...
def add_reminder():
    ...
    if not note or not time_str:
        messagebox.showwarning("Empty", "Please enter reminder and time.")
        return

    new_entry = {"time": time_str, "text": note, "medication": med}
    if selected_date in reminders:
    ...
    if date_str in reminders:
        for reminder in reminders[date_str]:
            if reminder["time"] == time_str:
                ...
                risk = medication_risks.get(med_name, "")
                prompt = f"Send a friendly reminder to the user: '{reminder['text']}'. Also mention any health risks: '{risk}'"

                try:
                    response = ollama.chat(model='llama2', messages=[{"role": "user", "content": prompt}])
                    message = response['message']['content']
                except:
                    message = f"{reminder['text']}\nRisk: {risk}" if risk else reminder['text']
