...
def parse_prescription_with_ai(text):
    prompt = f"""Extract medication schedule from this prescription text:
{text}
Return JSON like:
[
  {{
...
def scan_prescription(image_path):
    ...
    return parse_prescription_with_ai(raw_text)

def add_prescription_to_reminders(start_date, meds):
    start_dt = datetime.strptime(start_date, "%d/%m/%Y")
    for med in meds:
        for day in range(med["days"]):
            date_str = (start_dt + timedelta(days=day)).strftime("%d/%m/%Y")
            for t in med["times"]:
                reminders.setdefault(date_str, []).append({
                    "time": t,
                    "text": f"Take {med['medication']}",
                    "medication": med["medication"],
                    "risk": medication_info.get(med["medication"], {}).get("risk", ""),
                    "benefit": medication_info.get(med["medication"], {}).get("benefit", "")
                })
...
def upload_and_process():
    ...
    if not meds:
        messagebox.showwarning("No Medications Found", "Could not extract medications from the image.")
        return
    date = cal.get_date()
    add_prescription_to_reminders(date, meds)
    messagebox.showinfo("Success", f"Reminders scheduled for {date}:\n" + "\n".join([f"{m['medication']} for {m['days']} days" for m in meds]))
