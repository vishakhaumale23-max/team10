    ...
    if os.path.exists(REMINDER_FILE):
        ...
        sample = {
            today: [
                ...
                {"time": "16:46", "text": "Take Cetirizine", "medication": "Cetirizine"}
            ]
        }
...
# Add reminder section with dropdown
...
med_dropdown['values'] = list(medication_options.keys())
...
def add_reminder():
    ...
    if not note or not time_str or med == "Select Medication":
        messagebox.showwarning("Empty", "Please enter reminder, time, and select medication.")
        return
    ...
    if date_str in reminders:
        for reminder in reminders[date_str]:
            if reminder["time"] == time_str:
                ...
                risk = medication_options.get(med_name, "")
